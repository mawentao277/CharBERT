...
from processors.utils_pos import convert_examples_to_features, get_labels, read_examples_from_file
...
from modeling.modeling_charbert import CharBertForTokenClassification
from modeling.configuration_bert import BertConfig
from modeling.modeling_roberta import RobertaForTokenClassification
...
MODEL_CLASSES = {
    "bert": (BertConfig, CharBertForTokenClassification, BertTokenizer),
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    results = {
        ...
        "acc": num_acc * 1.0 / num_all
    }

    logger.info("***** Eval results %s *****", prefix)
    logger.info("num_all: %f num_acc: %f" % (num_all, num_acc))
...
def main():
    ...
    #print(f"the num_labels after init the config: {config.num_labels}")
    ...
    model = model_class.from_pretrained(args.model_name_or_path,
                                        ...
                                        config=config,
                                        cache_dir=args.cache_dir if args.cache_dir else None)
    #print(f"the num_labels in model: {model.num_labels}")
