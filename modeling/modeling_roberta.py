...
class RobertaModel(CharBertModel):
    """
    ...
    def __init__(self, config):
        super().__init__(config, is_roberta=True)
...
class RobertaForMaskedLM(BertPreTrainedModel):
    ...
    def forward(self, char_input_ids=None, start_ids=None, end_ids=None, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None,
                masked_lm_labels=None, adv_labels=None):
        ...
        prediction_scores, term_scores = self.lm_head(sequence_repr, char_sequence_repr)
        outputs = (prediction_scores, term_scores, ) + outputs[4:]  # Add hidden states and attention if they are here
...
class RobertaLMHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.char_dense = nn.Linear(config.hidden_size, config.hidden_size)
    ...
    def forward(self, features, char_features, **kwargs):
        ...
        cx = self.char_dense(char_features)
        cx = gelu(cx)
        cx = self.layer_norm(cx)
        cx = self.adv_decoder(cx)
        return x, cx
...
class RobertaForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.roberta = RobertaModel(config)
        self.char_classifier = nn.Linear(config.hidden_size * 2, self.config.num_labels)
    ...
    Examples::
        ...
        logits = self.char_classifier(seq_output)
...
class RobertaForQuestionAnswering(BertPreTrainedModel):
    ...
    Examples::
        ...
        """
        #print(f"char_input_ids shape in RobertaForQA: {list(char_input_ids.size())}")
        #print(f"input_ids shape in RobertaForQA: {list(input_ids.size())}")
        outputs = self.roberta(char_input_ids,
            start_ids,
            end_ids,
            input_ids,
            attention_mask=attention_mask,
