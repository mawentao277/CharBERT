    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                tmp_res = logits.detach().cpu().numpy()
                #print(f"tmp_res: {' '.join(map(str, tmp_res))}")
        ...
        #print(f"preds: {' '.join(map(str, preds))}")
        #print(f"out_label_ids: {' '.join(map(str, out_label_ids))}")
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if False:
        ...
        examples = glue_processor.get_dev_examples(args.data_dir) if evaluate else glue_processor.get_train_examples(args.data_dir)
    ...
    dataset = TensorDataset(all_char_ids, all_start_ids, all_end_ids, all_input_ids, all_attention_mask, all_token_type_ids, all_labels)
    return dataset
